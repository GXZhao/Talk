...
@interface XYXMPPTool ()<XMPPStreamDelegate, XMPPRosterDelegate, UIActionSheetDelegate>
{
    XYResultBlock _resultBlock;
}
...
- (void) setXmpp
{
    ...
    _xmppMsgArch = [[XMPPMessageArchiving alloc]initWithMessageArchivingStorage:self.xmppMsgArchStore];
    // 初始化自动重连
...
- (void) connectHost
{
    ...
    self.xmppStream.hostName = XYXMPPHOSTNAME;
    self.xmppStream.hostPort = XYXMPPPORT;
    NSString *userName = [XYUserinfo sharedXYUserinfo].userName;
    if ([XYUserinfo sharedXYUserinfo].registerType) {
        userName = [XYUserinfo sharedXYUserinfo].userRegisterName;
    }
    XMPPJID *myJid = [XMPPJID jidWithUser:userName domain:XYXMPPDOMAIN resource:@"iphone"];
...
- (void) sendPasswdToHost
{
    ...
    NSError  *error = nil;
    if ([XYUserinfo sharedXYUserinfo].registerType) {
        pwd =[XYUserinfo sharedXYUserinfo].userRegisterPasswd;
        [self.xmppStream registerWithPassword:pwd error:&error];
    }else{
        pwd =[XYUserinfo sharedXYUserinfo].userPasswd;
...
- (void) xmppStreamDidDisconnect:(XMPPStream *)sender withError:(NSError *)error
{
    ...
    if (error &&  _resultBlock) {
        _resultBlock(XYXMPPResultTypeNetError);
    }
}
/** 注册成功 */
- (void) xmppStreamDidRegister:(XMPPStream *)sender
{
    MYLog(@"注册成功");
    if (_resultBlock) {
        _resultBlock(XYXMPPResultTypeRegisterSuccess);
    }
...
{
    MYLog(@"注册失败");
    _resultBlock(XYXMPPResultTypeRegisterFaild);
}
/** 授权成功 */
- (void) xmppStreamDidAuthenticate:(XMPPStream *)sender
{
    MYLog(@"授权成功");
    _resultBlock(XYXMPPResultTypeLoginSucess);
    [self sendOnLine];
}
/** 授权失败 */
...
{
    MYLog(@"授权失败");
    _resultBlock(XYXMPPResultTypeLoginFaild);
}
/** 用户登录 */
...
- (void)xmppRoster:(XMPPRoster *)sender didReceivePresenceSubscriptionRequest:(XMPPPresence *)presence
{
    ...
    NSString *jidStr = [NSString stringWithFormat:@"%@@%@",presenceFromUser,XYXMPPDOMAIN];
